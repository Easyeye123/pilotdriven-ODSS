...
The same approved event must not be represented by multiple conflicting successor checkpoints. Idempotency is keyed by a stable transition identifier, not prose; repeated wording under a new source-revision or material-change event still creates a checkpoint. A successor requires checkpoint and human-record identifiers unique within the complete user chain, a later UTC time, the prior checkpoint-envelope fingerprint and a distinct verified human record. The existing chain plus candidate must pass full sequence, predecessor, identifier, timestamp, transition-semantic, transitive-digest, current-head and authority validation before any write. Only protocol version `1.0` is accepted by the v1 reference module; an unknown version fails closed rather than being interpreted with v1 semantics. The repository and path remain immutable; changes to the merged commit or policy fingerprint require an `APPROVED_SOURCE_REVISION` transition. Status or gap metadata changes require an `APPROVED_STATUS_CHANGE`. `ACTIVE` requires no gap reason, no unavailable layers and `safe_to_resume=true`; an activation call that still supplies unresolved gap metadata is rejected instead of silently discarding it. `INCOMPLETE` requires a controlled reason code semantically matched to its unavailable layers, at least one unavailable layer and `safe_to_resume=false`. A currently verified layer is shown as recovered. If other recorded gaps remain, the compatible original gap reason remains active; once all recorded gaps verify, the brief changes to `STATUS_CLEARANCE_PENDING` and remains gated by `approved checkpoint status clearance` until a new approved status-change checkpoint establishes `ACTIVE`. Genesis events are orthogonal: a mode event cannot also establish status state, a status event cannot also select mode, and other approved event types retain both defaults. A genesis status-change event cannot create a default-active no-op. `SUPERSEDED` is not a persistable v1 tail state because successor history itself records supersession. Source references, controlled facts, approved changes and superseded positions are append-only. A later correction adds a supersession record and corrected fact; it does not erase history. An `APPROVED_MEMORY_CHANGE` transition may deactivate or supersede pilot memory in the latest active view, but immutable predecessor checkpoints retain the historical value for audit. Genuine erasure requires a separately governed deletion or cryptographic key-destruction process outside this append-only checkpoint chain. The private store is append-only and immutable. Its compare-and-swap checks both the expected predecessor identifier and complete envelope fingerprint and atomically advances the independent monotonic head. Before a persistence path reports success—including an idempotent retry, rejected compare-and-swap recovery or unknown transport outcome—it obtains a bounded, mutually consistent chain/head snapshot and independently verifies the latest authority artifacts. A concurrent fast follower causes a fresh snapshot attempt rather than a false failure; a commit followed by transport failure is reconciled by verified candidate presence.
...
If both authority layers are accessible, retrieve the complete private chain, verify every predecessor envelope digest and semantic transition transitively, match its tail to the independently attested monotonic head, and obtain a fresh trusted-adapter receipt matching the repository, path, merged commit, authority fingerprints, governed-state fingerprint embedded in the human record and complete checkpoint-envelope fingerprint. Validate the pseudonymous user scope before passing it to a store adapter. Verifier return types are checked before reading the trusted clock; a contract-invalid return is an adapter failure, not evidence that an authority or clock was verified. The trusted clock is read only after each type-valid verifier return and preserves sub-second precision. If the clock succeeds but a head binding then fails, the brief records the clock as recovered while leaving the head unavailable. Reject expired receipts, a stale valid prefix, unsupported protocol versions, unknown or noncanonical schema fields, competing sequence numbers, reused identifiers or broken predecessor links. An inaccessible private store, an accessible store with no checkpoint, and an accessible but invalid chain are distinct structured recovery states; they must not all be reported as store failure. Resume only when the verified tail is `ACTIVE` and `safe_to_resume=true`; a verified `INCOMPLETE` tail produces a structured gated brief rather than a normal resume. The brief lists every verification stage already established before the failure, not merely the last candidate, and never lists the same layer as both recovered and unavailable. Once the underlying gaps are gone, an approved status-clearance transition remains required. An unmerged pull request is not active normative authority. If only one authority layer is accessible, a trusted adapter raises a sanitized typed layer failure so bootstrap can identify which layer was recovered and which is unavailable. Layer values and reason codes come from closed, non-overlapping, duplicate-free enumerations, and every reason is cross-validated against its named unavailable layer. Adapter-supplied recovered layers are also stage constrained: only the opposite authority layer may be claimed during a one-layer authority failure, and head, clock or status-clearance recovery cannot be asserted by an unrelated adapter. A head verifier cannot inject an authority-stage failure and vice versa; wrong-stage typed failures are normalized to the applicable adapter layer. Arbitrary adapter text cannot enter the UI-safe brief. Head and clock failures receive distinct opaque reason codes and component-specific recovery actions, regardless of which verification phase detects them. Public exception text and tracebacks must not reproduce raw store, stored-row, verifier or clock diagnostics, including on capability, decoding, read, compare-and-swap and post-write paths; controlled adapters own any private diagnostic logging. The host must render the safe brief and block substantive use of unverified or uncleared case state. If neither layer is accessible, the same structured brief states that no controlled checkpoint can be verified; model memory is not a substitute.
...
- an unknown compare-and-swap transport outcome is reconciled by verified candidate presence before success or failure is reported;
...
- every successful persistence path, including idempotent retry and race recovery, verifies the current chain/head snapshot and latest authority artifacts;
- a head advance between snapshot retrieval and verification triggers bounded reread rather than a false failure;
...
- repository and path pointers reject traversal, dot segments, backslashes, controls, empty segments and noncanonical whitespace;
...
- raw private-store exceptions are also sanitized on persistence read, compare-and-swap and post-write verification paths;
- private-store capability and stored-row decoding failures are sanitized, including controlled-error subclasses and secret-looking field names;
- one-layer authority failures and trusted-clock failures identify the exact unavailable layer with sanitized reason codes;
- typed recovery failures reject arbitrary reason codes, reason/layer semantic mismatches, wrong-stage unavailable or recovered-layer injection, and arbitrary, duplicate or overlapping layer labels;
- clock failures detected during either head or authority verification produce a clock-specific recovery action;
- failure briefs preserve the complete set of already verified prerequisite stages without recovered/unavailable overlap;
- invalid verifier return types fail at the adapter layer before another clock read, while a post-clock head-binding failure records the clock as recovered;
- missing checkpoints, inaccessible stores and malformed accessible chains identify distinct recovery layers;
- unsupported protocol versions fail closed and produce a structured bootstrap gate;
...
- a currently verified layer is not also displayed as unavailable, and persisted `INCOMPLETE` state requires approved status clearance before activation;
- a genesis status-change cannot encode a default-active no-op or smuggle a mode selection, and other genesis events cannot smuggle orthogonal status or mode state;
- invalid user-scope identifiers are rejected before private-store I/O;
- activation cannot silently discard caller-supplied unresolved gap metadata;
