...
15. Every successor binds the prior governed-state fingerprint so history is transitively authenticated.

## Integration boundary

`helpyou_continuity.py` is a reference enforcement module, not a hosted persistence service. Product startup must call `bootstrap_helpyou_session`, provide a `PRIVATE` compare-and-swap store adapter and a trusted authority verifier, retrieve and transitively verify the complete checkpoint chain, and render the returned status brief before the first substantive Helpyou response. The verifier must independently establish merged-main reachability, recompute both artifact hashes, confirm that the human record embeds the governed-state fingerprint, and issue a receipt valid for no more than 15 minutes. Bootstrap requests a fresh receipt; receipt expiry never deletes or expires the stored checkpoint. Candidate transitions are fully validated before compare-and-swap. The product must authenticate the user's approval and retain private approval evidence before calling an approved-change function; the module itself only rejects draft-labelled events. Documentation or a pull-request branch alone does not create platform-level automatic continuity.
